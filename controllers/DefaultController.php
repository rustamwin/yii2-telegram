...
class DefaultController extends Controller
{
    ...
    public function actionInitChat()
    {
        ...
        if(!$session->has('tg_chat_id')) {
            if (isset($_COOKIE['tg_chat_id'])) {
                $tlgrmChatId = $_COOKIE['tg_chat_id'];
                $session->set('tg_chat_id', $tlgrmChatId);
            } else {
                $tlgrmChatId = uniqid();
                $session->set('tg_chat_id', $tlgrmChatId);
                setcookie("tg_chat_id", $tlgrmChatId, time() + 1800);
            }
        }
...
//            $commandsPath = realpath($basePath . '/Commands/SystemCommands');
            $commandsPath = realpath($basePath . '/commands/user');
